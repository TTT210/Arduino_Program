...
const int rightDuty = 100;
const int leftDuty = 30;
...
void setup() {
  ...
  count_pulse = 0;                        // 初期値設定
  vol_rpm = 0;                            // 初期値設定
  duty = constrain(duty, 0, 255);      // dutyの範囲を-255から255に定義→通常
  //duty = constrain(duty, -255, 80);      // dutyの範囲を-255から安全な範囲に定義→制御がうまくいかず急に速度が出るのを防ぐため
                                          //→うまく制御できなくなることがあったので廃止
